...
def clean_name(feature):
    replacements = {
        ...
        'ca_0.0':                 'No major vessels affected (ca=0)',
...
if st.button("Predict Risk", type="primary", use_container_width=True):
    ...
    st.markdown(f"""
The model predicts a **{risk_label} risk** of heart disease for this patient
with a probability of **{prob:.1%}**.
 
The average patient in this dataset has a **{base_val:.1%}** baseline risk.
This patient's specific clinical values shifted that baseline to **{prob:.1%}**.
    """)
 
    if pushing_up:
        st.markdown("**Factors increasing risk ↑**")
        for feat, val in pushing_up:
            st.markdown(f"- **{clean_name(feat)}** pushed risk up by {abs(val):.2f} points")
 
    if pushing_down:
        st.markdown("**Factors reducing risk ↓**")
        for feat, val in pushing_down:
            st.markdown(f"- **{clean_name(feat)}** pushed risk down by {abs(val):.2f} points")
